...
**Dashboard** — a personalized feed (pending join requests, upcoming check-ins, flagged tasks, pending invites, and needs-action items for whoever owns Budget/Event scheduling/Shifts/Conflict management), always cross-cycle regardless of the nav switcher's scope, plus a community snapshot (tier composition, branch spread, and per-branch health genuinely scoped to the active view-scope cycle) with a general/this-cycle toggle on the active-member count — the union across every currently-open cycle, or just the one the switcher is narrowed to — and, for a member who hasn't finished or skipped it, an onboarding panel.

**Member onboarding** — a first-session nudge, never a gate: a handful of static orientation cards, any onboarding-tagged Profile questions still unanswered, a short set of trait-axis questions (e.g. wants direction vs. independence, desk work vs. physical), and 2-3 open tasks surfaced with a link out to the full board — ranked by proximity across whatever trait axes both the member and a task have set, plus tag overlap and satisfied requirements, never shown as a score and never assigning anyone automatically. The same heuristic surfaces a "you might also like" strip once a task is marked done. Tags stay a guided (autocomplete-suggested) but free-form vocabulary for search/filtering; languages get their own repeatable field with a proficiency level, checked by a task's language Requirement. The rest of the trait axes, and a member's spoken languages, are editable any time at `/profile`; the axis vocabulary itself is admin-managed at `/settings`.

**Forms & feedback** — a shared form primitive (free text/single/multi-choice) reused across post-cycle feedback, recruitment applications, and more, authored through a real settings-screen builder (add/remove/reorder fields, live preview, inline validation) rather than hand-typed config — the same builder edits a Form's fields or a Profile question's shape after creation too, never just at first authoring. A field can also be tagged as mapping to a once-ever Profile question, so an answer given on, say, a recruitment application seeds that new member's real profile directly at conversion — reviewed as one compact, editable summary at onboarding rather than asked a second time.
