...
Every year, at Peach Please as much as anywhere, the same thing happens: a handful of people — sometimes really just one or two — end up carrying far more than their share. Not because nobody else is willing. Usually it's simpler and sadder than that: there's no clean way for "I have an hour and I'm handy with a drill" to find "we need someone to fix the shade structure" without going through that one person first. People don't know what needs doing, the person coordinating ends up spending more time chasing status updates than the work itself would take, and nothing becomes visible until it's already a crisis. A task quietly stalls for weeks and nobody notices until it's too late to fix calmly.

There's a quieter, more specific version of this too. Someone takes on a role because they're genuinely excited about a piece of what it involves — they love menu planning, say — and the role comes bundled with everything else that's traditionally attached to it as well. Ideally they'd hand off the parts that aren't their thing. In practice, a fixed role doesn't have a clean way to delegate just a slice of itself — there's no unit smaller than "the whole role" to actually hand someone. So the parts nobody's excited about quietly don't get done, or they land on whoever's already paying enough attention to notice.

And a lot of what actually makes camp run — how the kitchen system works, what the dome build actually involves, who to ask about what — lives only in a few people's heads. When those people step back, even for a year, it goes with them.
...
Most ways of organizing a group start from *positions* — you're the kitchen lead, you're on the build team. Orchard starts from *tasks* instead. A task is a real, sized, ownable piece of work with a clear picture of what "done" looks like. Roles still exist, but only as a description of what someone currently owns — a snapshot, not a fixed job that follows them around forever.

That also dissolves the bundling problem from above: the parts of a role nobody's excited about are just separate tasks now, sitting on the board where someone else can actually see them and pick them up — not silently glued to a role someone else is already busy with.
