...
- ✅ `/community` — the Community group's hub at `/community`, with its own links to Members/Assemblies/Events/Settings, an Invite-a-member action when Recruitment is enabled, and a compact community snapshot. `/members` is intentionally only the member directory; it is not a second Community hub.
