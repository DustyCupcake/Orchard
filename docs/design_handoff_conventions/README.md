...
- ✅ `/members` — now "the main community view" the same way `/board` is for Tasks: a button row to Messages/Assemblies/Documentation/Cycles (+ Feedback once a standing form is configured) sits under the heading, mirroring Board's hub-button pattern exactly.
