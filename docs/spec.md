...
**A member's view scope is either every open Cycle they've declared Participation `coming` for, or one Cycle they've deliberately narrowed to** — open or closed, doesn't matter, since looking at a closed Cycle's history is a normal thing to do. The aggregate ("all the cycles I'm actually coming to") is the default; narrowing to one specific Cycle is a real, sticky, persisted choice, and it only ever changes when a member deliberately makes it — never as a passive side effect of following a link or landing on a URL that happens to name a different Cycle. A shared link that does carry a specific Cycle offers to switch the recipient's view scope to it, but always asks first; it never moves silently out from under them.

Different mechanics genuinely need different shapes of this, and which shape applies is a deliberate per-mechanic choice, not a rule applied uniformly everywhere:

- **Some things are inherently about exactly one Cycle at a time** and read the narrowed scope directly — a Participation declaration, a Budget cycle. When the view scope is still the aggregate and it covers more than one open candidate, these ask plainly which Cycle is meant rather than guessing at one.
- **Some things blend a Cycle's own scoped items with Cycle-independent ones that apply regardless** — the board's tasks, a member's own action items on the Dashboard. A task tied to a specific Cycle only shows within that Cycle's own scope; a Cycle-less task — evergreen work, or anything that predates Cycles altogether for a Community that only turned them on partway through its life — always shows, unfiltered by whatever the scope happens to be narrowed to.
- **Some things are fully independent per Cycle, all the way down to who's authorized to run them** — Event scheduling's programme and Spatial planning's layout each have their own designated owner *per Cycle*, exactly like Budget already does. A Reunion weekend's own programme owner never inherits authority over the main Season's programme just because they happen to hold a similarly-named task, and the reverse is equally true. A brand-new Cycle simply starts with no owner of its own for either — the same way a brand-new Cycle starts with no Budget process until someone actually sets one up.

**A member's own action items are never filtered by view scope.** Someone holding tasks across two concurrently-open Cycles sees what's next on them from both at once, on the Dashboard, regardless of which single Cycle (if any) their browsing view happens to be narrowed to right now — narrowing down to look at one Cycle's board is never allowed to quietly hide something the member still genuinely needs to act on. The community-wide numbers the Dashboard also shows (see Community snapshot, below) work the other way around on purpose: those are about the group, not any one member's own obligations, so they're allowed — and expected — to change shape depending on whether the viewer means "the community in general" or "this one specific Cycle."
...
| Entity         | Key fields                                                                                                                                                                                              | Notes                                                                                                                                                                            |
|----------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| MenuPlan       | id, community_id, cycle_id (nullable — null = the evergreen "standing menu"), status (enum: draft, published), created_by                                                                                 | one per scope; publishes once, reads for everyone; no owner column — authority comes from the `kitchen` permission grant (Access follows the task)                                    |
| Meal           | id, menu_plan_id, label, date, phase_id (nullable), head_count (nullable), shift_series_id (nullable), notes                                                                                             | head_count = expected eaters, the recipe-scaling baseline                                                                                                                            |
| Dish           | id, meal_id, name, description, serves (nullable), allergen_flags (text set), dietary_note, source (enum: seed, from_idea), source_idea_id (nullable), added_by                                          | a dish is a recipe; allergen flags are a planner signal, never a medical guarantee — see Sensitive data                                                                              |
| DishIngredient | id, dish_id, name, amount, unit, sort_order                                                                                                                                                              | recorded at recipe-as-written scale; scaling (`head_count / serves`) happens on read, never written back                                                                              |
| FoodIdea       | id, menu_plan_id, kind (enum: recipe_suggestion, dish_request, preference), title, body, suggested_by, status (enum: open, adopted, declined), adopted_dish_id (nullable), decline_reason (nullable)     | attributed, no anonymity; adopting creates the Dish and flips the idea in one action                                                                                                 |
...
- **Coordination comes in widths, and a community picks.** A coordinator is whoever currently holds a task granted for coordination — the grant is a real task on the board, not a rank (see Admins for the same pattern). What that grant *covers* follows from where the task sits, and there are three deliberately distinct answers, because "who coordinates" has no single right width:

  - **Per branch, across every event** — an event-independent coordination task in branch B covers every task in branch B, whichever event it belongs to. The standing default.
  - **Per event** — a coordination task placed in event C covers all of event C, across every branch. The answer for a small event that doesn't have a branch structure worth splitting up.
  - **Not branch-scoped** — the same two rules, except the branch is never consulted. Left outside every event, it covers the whole community; placed in event C, it covers event C and nothing beyond it, exactly as the row above does. The answer for a community too small, or too informal, to want a coordinator per branch: one grant instead of one per branch.

  These compose: a community can run per-branch, per-event and community-wide coordinators side by side, and one person's coverage is the union of the tasks they hold. Worth stating why the third exists separately rather than as a flavour of the first: a branch is the *smallest* unit a task can be placed in, so no arrangement of ordinary tasks can express "every branch" — a task that isn't in a branch doesn't exist. The trade is deliberate: in exchange for never being narrowed to a branch, this one is also never narrowed to an event if you don't place it in one.
...
- **Escalation** — unplaceable tasks surface in a shared "needs an owner" view visible to all coordinators, with cross-branch placement encouraged. Taking a task is always a visible, deliberate act, never a silent default. A coordinator's *coverage* is broad by design, but the board does not name them against every task they could touch — a name on a card means somebody needs to act on that task, so it appears on the escalated queue and on unclaimed criticals (alongside the Backstop marker, which triggers on the same condition), and nowhere else. Coverage quietly governs which actions a coordinator is offered instead.
...
Menu planning and cooking, generalized from the original design's **"Food & drinks (Fruit)"** branch — the one piece of that design the genericization dropped outright rather than deferred, so it's restored here as a first-class optional module (the restoration trail and build plan live in `docs/food-drinks-module-plan.md`). Fully independent per Cycle like Budget and Event scheduling, with one structural distinction up front: kitchen authority comes from **whatever tasks the Community grants the `kitchen` permission module to** (Access follows the task, see Transparency & access below), and the cooking side runs on ShiftSeries coverage rather than one-shot tasks — the kitchen is as much recurring, never-"done" work as it is a deliverable.

- **Menu plan** — one per scope: the current Cycle if one is running, or the community's evergreen "standing menu" if it isn't (same per-Cycle independence as event scheduling, see Concurrent cycles & view scope — a community running two concurrent Cycles gets two independent menus). Draft → published, gated the same way an event programme is: only a current holder of a kitchen-granting task can build or publish; every member reads the published menu. Deliberately no owner column on the plan — authority rides the grant, and the grant rides the task, so access follows whoever currently holds one of those tasks, the same principle as every other access decision below.
- **Meals and dishes — the food schedule** — a meal is one dated row (a plain absolute date for v1, with an optional phase anchor); each meal lists its dishes. Each dish carries a small fixed allergen-flag set plus a free-text dietary note — always a *signal for the planner*, never a medical guarantee. Member allergy records feed the constraint view only through the Sensitive data module's field→grant route, with the same per-read consent re-check as everywhere else (see Sensitive data, below).
- **Recipes that scale to headcounts** — every dish is a recipe underneath: a serving count plus ingredients with amounts and units, recorded at recipe-as-written scale. Each meal carries a planner-entered head count (expected eaters). The menu renders per-dish scaled amounts and a per-meal **purchasing list** grouped by exact ingredient name — derived on read, never stored, so the numbers the ordering task works from come straight off the menu instead of being guessed twice.
- **Participatory input — "Suggest something"** — any member files a food idea against the active menu plan (recipe suggestion, dish request, or explicit preference; attributed — no anonymous suggestions, the same posture as task comments). The kitchen task holder reviews the inbox and either adopts an idea into a real dish or declines it with a reason. Deliberately separate from Input rounds: an idea is addressed to the menu, not polled from the group — though an owner can still pose an Input-round poll on top when it's actually called for.
- **Ordering & equipment** — ordinary tasks, surfaced as a "supply" lens on the menu page: food-supply tasks within the menu's scope (however the Community tags them) show beside the menu with their Task resources visible. One database, many lenses — no new storage.
- **Cooking** — a ShiftSeries per meal where the Community runs shifts, so the roster for "Friday dinner" is one click from the meal. The link is optional: a ShiftSeries remains ordinary recurring coverage with no menu link when that's all it is (see Shifts / rota, below).
...
Health, allergies, orientation, emergency contacts. GDPR Art. 9 handling if enabled. Off by default. Field-level access is purpose-bound rather than role-bound — a Community defines which task, tier, **or permission grant module** unlocks which field (e.g. only whoever holds the catering coordination task sees allergy data; only the designated safety contact sees health conditions), consistent with the "access follows the task" principle below. The grant-module route is the multi-task answer to that same principle: a Community links a field to a permission grant **once** (say, `allergies` → the Kitchen module's grant), and any current holder of a task carrying that grant — however many tasks the Community grants — sees the field, with no per-task rules to keep in sync as tasks get granted or un-granted. Allergies feed the Kitchen menu's constraint view through exactly this route, and through nothing else (see Kitchen, above). Turning a given field on for a member requires an active, non-withdrawn `ConsentRecord` against the matching `ConsentPurpose` (see *Member contact & privacy*, below, in the data model) before the field is populated or made visible to anyone — access stays purpose-bound on top of that, not instead of it, and withdrawing consent has to actually revoke access, not just flag it.
...
Optional (per Community, off by default except where noted): recruitment, budget, event scheduling, spatial planning, kitchen, sensitive-data, conflict management, assemblies, shifts/rota, **documentation (on by default)**.
...
**Active member count has two honest readings once more than one Cycle can be open at once** (see Concurrent cycles & view scope, above), and the panel offers a plain toggle between them rather than picking one: the union of everyone declared `coming` across every currently-open Cycle, regardless of the viewer's own view scope — always available, the closest thing to "how many people, period" — or `coming` for just the one Cycle the viewer's view scope is actually narrowed to, which is only ever a real option once it's narrowed that far; while the view scope still covers more than one Cycle, there's nothing for that second reading to mean yet, so the toggle simply doesn't offer it rather than guessing. Composition (Tier/Branch spread) and community-average contribution stay exactly as broad as they've always been — neither is Cycle-specific to begin with.

**Branch health.** The same panel carries a per-branch rollup of task attention levels — the concrete answer to "is coordination actually happening here," computed entirely from state the lifecycle mechanics already produce (see **Lifecycle & attention**), nothing separately tracked. Unlike active member count above, this one doesn't get its own general/this-Cycle toggle — it simply follows the viewer's own view scope directly, the same way the board does: a task tied to a Cycle only counts toward its branch's rollup while that Cycle is part of the viewer's scope, and a Cycle-less task always counts, regardless. Two tiers of the same underlying data, the same "signal without exposing a number nobody agreed to publish" split already used for capacity visibility:
