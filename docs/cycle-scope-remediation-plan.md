...
| **Cycle-shaped** — authority targets per-cycle data | `event_scheduling_owner`, `spatial_planning` (already), `branch_coordination` (new), `announcements` (new cycle variant), `backstop` (new), `shift_management` (new — cycle roster; community variant = standing series) | Placement is the declaration: in cycle C → that cycle; cycle-less → the community's cycle-less scope. |
| **Cycle-shaped, deferred** — need cycle-keyed data first (§6 D4) | `feedback_review`, `recruitment` | Stay community-wide this pass; scoped in a later pass once responses/intake carry a cycle. |
...
- `feedback_review`: **deferred** with recruitment (§6 D4) — stays community-wide this pass. The cycle-scoped resolution is specified here for the later data pass (§4.3): once `formResponse` carries a cycle, resolve the reviewer against the target *response's* cycle the same way the two Phase 68 modules resolve against their item's cycle.
...
**Fix:** the data layer must know which cycle an item belongs to before authority can "remain in the cycle."

- `formResponse` — add `cycleId` (spec already lists `response.cycle_id`: `docs/spec.md:867`) as part of the **deferred** feedback_review pass (§6 D4), not this one.
- `conflictReport` — deliberately left **unkeyed** (conflict stays community-level, §2.2).
- `recruitment` — cycle-linked applications are the plan's own "genuinely unresolved" item (§1, `docs/development-plan.full-archive.md` "Beyond"). **Decided (D4):** defer — inquiries/applications get a cycle association first, then recruitment scopes in the same later pass as feedback_review.
- Events, plots, budget, participation, tasks — already cycle-keyed (`eventProposal.cycleId`, `Plot.cycleId`, `BudgetCycle.cycleId`, `Participation.cycleId`, `task.cycleId`).

**Files:** `src/db/schema/form.ts`, `src/lib/forms.ts`. **Effort:** Small–Medium.
...
- **D4 — Land the ready five now.** `event_scheduling_owner`, `spatial_planning`, `branch_coordination`, `announcements` (cycle variant) and the three community-shaped modules (`admin`, `conflict_team`, `support`). `feedback_review` and `recruitment` wait for their cycle-keyed data (§4.3), then scope in a later pass.
...
**Open:** none outstanding — Q5 (column fate → single migration, D8), Q8 (roster = placement), Q9 (grant-everywhere), Q10 (single per scope), and Q11 (`shift_management`) are all settled. What remains are implementation details tracked inside the work items — e.g. exactly how occurrence materialization defers when a cloned cycle's dates aren't known at clone time (§4.8).

---

## Work plan (suggested order)

1. **Backend model** — §4.1 + §4.2, with `task.cycleId` as the only scope read; existing Phase 68 tests must pass unchanged post-migration.
2. **Cycle clone grants** — §4.4 (unblocks the "clone carries authority" story; needed before any UI depends on it).
3. **Backstop** — §4.7 + §5.5 (new module, cycle-creation default, unclaimed-critical marker + duty view). Rides directly on the unified resolver shape from step 1.
4. **Shift rosters** — §4.8 + §5.6 (series placement column, `shift_management` module, roster open-act + proposal gate, surfaces + clone carries the roster). Same resolver shape.
5. **Announcements cycle variant** — §4.5 (cycle send scope with selectable `coming`/`maybe` segments, D3).
6. **Interface** — §5.1 then §5.2 (the two confusing surfaces), then §5.3–5.4.
7. **Retire the old scope pipe** — §4.6, then drop the column in step 1's single migration (D8).
8. **Later pass (D4)** — `formResponse.cycleId` + cycle-keyed recruitment intake (§4.3), then scope `feedback_review` and `recruitment`.
