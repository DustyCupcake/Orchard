...
**The third case — coordination without a branch.** The two rules above leave a gap the original model couldn't reach: a *small community* that doesn't want a coordinator per branch. The cycle row (row C) is the answer only when the event has exactly one cycle; a community running several branches together had no way to say "one person coordinates all of it" without either a cycle-less task per branch (the very per-branch duplication being avoided) or one branch's column silently standing in for the whole community. Note this is a gap in *expressiveness*, not in placement: `task.branchId` is `NOT NULL`, so a cycle-less task always lands in exactly one branch and can therefore only ever be branch-wide. No placement can express "every branch".

**Locked — D15:** whole-community coordination is a **second coordination module key**, `community_coordination`, not a scope column on `permission_grant`. A `scope` column would reintroduce the second scope read that D8 just retired, and the gain is one extra row in the same capability-per-key shape the table already uses.

The only thing that makes it different from `branch_coordination` is that **the branch is not part of its scope** — so its scope rule is the ordinary one, unchanged: placement is the scope. A grant placed in event C is that event's coordinator and nothing more (strictly, per D1, exactly as a cycle-placed `branch_coordination` task behaves); only a grant with **no event** goes community-wide, and in that case the branch on its task is simply not read. It is therefore **cycle_variant**, not a community-only role — a community that wants a per-branch coordinator *and* a community-wide one can have both, and the tier table says so. It is **multi-cardinality**. Both keys resolve as one family in `src/lib/coordination.ts` (`COORDINATION_MODULE_KEYS`), and the resolver reads placement *first*, module second — a cycle-placed grant is a cycle row whichever key granted it, so `communityWide` can only ever be set by a grant with no cycle. `listCoordinationScopeIds` reports that case as a flag rather than enumerating ids, so the board, escalation queue, and view-scope gates stay O(1).
...
**Fix:** `/coordination` and `/escalation` gate on the view-scope cycle (via the `[cycleScope]` segment) when the community runs cycles, staying community-wide for cycle-less ones. Board badges resolve from both dimensions (§2.4): a task card flag says "coordinated by {name}" when the holder covers its branch column *or* its cycle row *or* is a cycle-less `community_coordination` holder (D15). `listCoordinationBranchIds` becomes `listCoordinationScopeIds`.

**Corrected after the fact — the badge is an attention signal, not a coverage readout.** As first written above, the badge fired whenever the holder *covered* the task, which is the same predicate that gates the card's Escalate action. Those are not the same question, and conflating them made one branch-wide coordinator stamp their name on **every task in the branch** — column semantics cover a branch across all its cycles, so coverage is true for essentially the whole branch and the badge carried no information on any of it. The badge is now gated on `needsNamedCoordinator` (`src/app/(app)/board/coordination-tag.ts`): the escalation queue (`attentionLevel = "escalated"`) plus unclaimed criticals — the same two populations that name a responsible party anywhere else in the app, and the exact trigger of the neighbouring "Backstop: {name}" marker (D5). Coverage is untouched and still drives the actions. The rule worth keeping: *a tag that names a person must be earned by a task needing one.*
...
**How "once" was actually achieved.** Stating the rule once per module isn't the same as stating it once on the page, and the first pass at this item did the former: fourteen modules, each hint leading with the same "a task grants what it sits in" clause, ~600 words of near-identical paragraph on one tab. The settings tab is now **grouped into two sections** — "Community-wide" and "Per-event" — with the shared placement rule stated once in each section header, and `PERMISSION_MODULE_HINTS` reduced to what holding each role *does*. The split is **derived from the tier table** (`PERMISSION_MODULE_SECTIONS`, filtered off `PERMISSION_MODULE_KEYS`) rather than hand-listed, so a new module can't land in no section or two; `tests/permissions.test.ts` asserts the sections partition the key set. Only the `community` tier earns its own section — `cycle` and `cycle_variant` share the per-event one, because the rule a reader needs is identical for both and the only difference is the row label `describeGrantScope` already prints. The task-detail checkbox list, which is *not* sectioned, instead shows the derived scope under each checkbox (§5.2's "checkboxes plus a scope line").
...
- **D15 — Whole-community coordination is a module key, not a scope column** (§2.4). `community_coordination`, **cycle_variant** (so placement still means the event, and a cycle-placed grant is that event's coordinator only), multi-cardinality, resolved as one family with `branch_coordination` and reported as a `communityWide` flag rather than expanded to every branch id. The one difference from `branch_coordination` is that the branch is not read. `task.branchId` being `NOT NULL` is what makes this a second key rather than a placement; a `permission_grant.scope` column would undo D8's single scope read.
